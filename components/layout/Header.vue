<template>
    <div class="menubase">
        <div ref="menuwrapper" class="w-full text-white flex items-center h-16 px-4 md:px-12 md:h-24 fixed z-50 top-0 left-0 ">
            <div class="logo">
                <NuxtLink to="/" class="text-xl flex gap-2 items-center" v-on:click="menuClose">
                    <div class="circles flex gap-1">
                        <span class="w-3 h-3 md:w-4 md:h-4 bg-[#14cac4] rounded-full flex"></span>
                        <span class="w-3 h-3 md:w-4 md:h-4 bg-[#fc2e79] rounded-full flex"></span>
                        <span class="w-3 h-3 md:w-4 md:h-4 bg-[#fbb635] rounded-full flex"></span>
                    </div>
                    <div class="text-logo text-[15px] md:text-lg">
                        <span class="font-light text-ld-300">Léo</span><span class="font-bold text-ld-100">DESIGAUX</span>
                    </div>
                </NuxtLink>
            </div>
            <nav class="ml-auto flex gap-4 text-xs md:text-sm text-ld-400">
                <LangSwitcher />
                <a v-on:click="menuhandler" class="hover:text-ld-50 transition hover:transition-none duration-500" to="/about"><span class="cursor-link">
                        <Slotmachine title1="MENU " title2="CLOSE" ref="menuSlot" title2Class="text-ld-50" />
                    </span></a>
            </nav>
        </div>
        <div ref="submenu" class="submenu w-full bg-primary text-white flex  items-center h-0 overflow-clip top-16 md:top-24 fixed z-50 left-0 right-0 bg-[#0a0919CC] backdrop-blur-sm">
            <div class="flex h-full w-full relative flex-col xl:flex-row">
                <NuxtLink v-on:click="menuhandler" to="/about" class="h-full w-full p-4 flex items-end relative transition-all menu-link group overflow-hidden">
                    ...
                    <span class="menu-text shrink-0 text-2xl md:text-4xl text-ld-400 transition-colors group-hover:text-[#ffffff] ">
                    ...
                    <span class="vertical-line origin-bottom-left absolute bottom-0 md:top-0 right-0 h-[1px] w-full md:h-full md:w-[1px] bg-[#ffffff30] content-[''] z-10"></span>
                </NuxtLink>
                ...
                <NuxtLink v-on:click="menuhandler" to="mailto:hello@leodesigaux.com" class="h-full w-full p-4 flex items-end relative transition-all menu-link group overflow-hidden">
                    <div class="opacity-0 hover:opacity-100 transition duration-500 absolute inset-0 h-full w-full bg-gradient-to-tr from-[#FFFFFF00] to-[#FFFFFF08]"></div>
                    <span class="menu-text shrink-0 text-2xl md:text-4xl text-ld-400 transition-colors group-hover:text-[#ffffff]">
        ...
        <div ref="overlaymenu" v-on:click="menuClose" :class="{ 'h-screen': menuOpened, 'h-0': !menuOpened }" class="duration-1000 overlay w-full bg-primary text-white flex items-center overflow-clip top-0 fixed z-40 left-0 right-0 transition-all bg-[#0a091999] md:bg-[#0a091933] backdrop-blur-sm"></div>
    </div>
</template>
<script setup>
...
const buildTransition = () => {
    nextTick(() => {
        ...
        tl.fromTo(".horizontal-line", {
            ...
            scaleX: 1,
            duration: 0.6,
        }, window.isMobile ? 1.5 : 0.5);
        tl.fromTo(".vertical-line", {
            scaleY: window.isMobile ? 1 : 0,
            scaleX: window.isMobile ? 0 : 1,
        }, {
            scaleY: window.isMobile ? 1 : 1,
            scaleX: window.isMobile ? 1 : 1,
            stagger: 0.3,
        }, 0.5);
...
watch(locale, buildTransition, { immediate: false });
